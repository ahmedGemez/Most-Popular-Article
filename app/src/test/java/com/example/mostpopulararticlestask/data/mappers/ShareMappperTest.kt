...
class ShareMappperTest{
    ...
    @Before
    fun setupTest() {
        article = Article("","U.S.","source","title","type","abstract")
        mostPobularData = MostPobularData("Copyright (c) 2021 The New York Times Company.  All Rights Reserved.",20,listOf(article),"")
    }
    ...
    fun toMostPobularData() {
        ...
        val mostPobularRemoteData = Gson().fromJson(jsonString, MostPobularRemoteData::class.java)
        shareMappper = ShareMappper()
        val result = shareMappper.toMostPobularData(mostPobularRemoteData)
        Assert.assertEquals("U.S.",result.articles[0].section)
